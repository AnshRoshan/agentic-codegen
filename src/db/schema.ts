...
export type Entity = {
  name: string; // singular PascalCase e.g. "Contact"
  plural: string; // e.g. "contacts"
  slug: string; // e.g. "contacts"
  fields: EntityField[];
};
...
export type Architecture = {
  ...
  components: Array<{
    name: string;
    type: "frontend" | "backend" | "database" | "infra" | "service";
    description: string;
    dependencies: string[];
  }>;
  dataFlow: string[];
...
export const projects = pgTable("projects", {
  ...
  mode: text("mode").notNull().default("greenfield"), // greenfield | brownfield
  ...
  costMicros: integer("cost_micros").notNull().default(0), // USD * 1e6
...
export const agentMessages = pgTable("agent_messages", {
  ...
  agentRole: text("agent_role"),
  kind: text("kind").notNull().default("info"), // info | tool | file | success | warning | error | user
...
export const hitlCheckpoints = pgTable("hitl_checkpoints", {
  ...
  type: text("type").notNull(), // schema | secrets | deployment | destructive
  title: text("title").notNull(),
  description: text("description"),
...
export const aiSettings = pgTable("ai_settings", {
  ...
  temperature: integer("temperature").notNull().default(20), // x100
